...
# Global variable (current coordinates)
current_actual = None
            ...
            if (numbers[0] == 0):
                if (len(numbers) > 1):
                    for i in numbers[1:]:
                        alarmState = False
                        if i == -2:
                            print("Robot Alarm: Robot Collision ", i)
                            alarmState = True
                        if alarmState:
                            continue                
                        for item in dataController:
                            if i == item["id"]:
                                # Changed zh_CN to en for English description
                                print("Robot Alarm: Controller errorid", i, item["en"]["description"])
                                alarmState = True
                                break 
                        if alarmState:
                            continue
                        for item in dataServo:
                            if i == item["id"]:
                                # Changed zh_CN to en for English description
                                print("Robot Alarm: Servo errorid", i, item["en"]["description"])
                                break  
                        
                    choose = input("Input 1 to clear error and continue running: ")     
                    if int(choose) == 1:
